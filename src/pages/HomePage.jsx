...
=======
import { BookOpen, ArrowRight, Megaphone, Calculator, LineChart, Globe, Landmark, Ship, GraduationCap, PiggyBank, Wallet, Rocket, Quote } from "lucide-react";
import Navbar from "@/components/layout/Navbar";
import Footer from "@/components/layout/Footer";
  ...
  return (
    <div className="min-h-screen bg-black text-white">
      ...
      <section className="relative min-h-screen flex items-center pt-20 overflow-hidden bg-black">
        <div className="relative z-10 max-w-7xl mx-auto px-6 w-full grid lg:grid-cols-2 gap-8 items-center">
          {/* Left: text */}
          <div className="animate-fade-up">
            <div className="flex items-center gap-3 mb-4">
              <div className="w-8 h-px bg-gold/50" />
            ...
            <h1 className="font-heading font-bold leading-[1.02] mb-5" style={{ fontSize: "clamp(2.8rem, 7vw, 6rem)", textShadow: "0 0 40px rgba(212,175,55,0.25)" }}>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link to="/archive" className="btn-shine-wrap flex items-center justify-center gap-2.5 px-7 py-4 text-xs tracking-[0.2em] uppercase font-semibold transition-all duration-300 hover:scale-105 border border-gold text-gold bg-gold/8 hover:bg-gold/15">
                <BookOpen size={14} /> Browse All Editions
              </Link>
              {featured && (
                <Link to={`/read/${featured.id}`} className="btn-shine-wrap btn-pulse-gold flex items-center justify-center gap-2.5 px-7 py-4 text-xs tracking-[0.2em] uppercase font-semibold transition-all duration-300 hover:scale-105 border border-gold/40 text-gold hover:bg-gold/10">
                  View Latest Edition
                </Link>
              )}
            </div>
      ...
      <section className="py-24 px-6">
        <ScrollReveal className="max-w-7xl mx-auto">
          <SectionLabel>What We Cover</SectionLabel>
          <h2 className="font-heading text-4xl md:text-5xl font-bold text-white text-center mb-4">Commerce &amp; Beyond</h2>
          <p className="text-white/40 text-sm text-center max-w-xl mx-auto mb-14">
            Covering every facet of the business world — from classroom theory to real-world markets.
          </p>
          <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {topics.map((t, i) => (
              <ScrollReveal key={t.num} delay={i * 80}>
                <div className="group bg-black/40 border border-gold/8 p-6 hover:border-gold/30 transition-all duration-500 relative overflow-hidden h-full">
                  <span className="absolute top-4 right-4 font-heading text-3xl font-bold text-gold/8 group-hover:text-gold/20 transition-colors">{t.num}</span>
                  <div className="w-11 h-11 rounded-full bg-gold/10 flex items-center justify-center mb-4 group-hover:bg-gold/20 transition-colors">
                    <t.icon className="text-gold" size={20} />
                  </div>
                  <span className="text-gold/40 text-[10px] tracking-[0.25em] uppercase block mb-1">{t.cat}</span>
                  <h3 className="font-heading text-white text-lg font-semibold mb-2">{t.title}</h3>
      ...
      {/* Featured Edition Showcase */}
      {featured && (
        <section className="relative py-28 px-6 overflow-hidden bg-navy/20">
          <div className="absolute inset-0 pointer-events-none">
            <div className="absolute top-1/2 left-1/2 rounded-full" style={{ width: "900px", height: "900px", transform: "translate(-50%, -50%)", background: "radial-gradient(circle, rgba(212,175,55,0.06) 0%, transparent 55%)", filter: "blur(80px)" }} />
          </div>
          <div className="relative max-w-7xl mx-auto grid lg:grid-cols-2 gap-16 items-center">
            <div className="relative flex justify-center order-2 lg:order-1">
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                <div className="rounded-full" style={{ width: "500px", height: "500px", background: "radial-gradient(circle, rgba(245,166,35,0.25) 0%, rgba(200,134,10,0.1) 30%, transparent 65%)", filter: "blur(60px)" }} />
              </div>
              {featured.cover_image && (
                <img src={featured.cover_image} alt={featured.title} className="relative w-72 md:w-80 lg:w-96 aspect-[3/4] object-cover border border-gold/20 animate-fade-up" style={{ boxShadow: "0 40px 100px rgba(0,0,0,0.8), 0 0 150px rgba(212,175,55,0.2)" }} />
              )}
            </div>
            <ScrollReveal className="order-1 lg:order-2">
              ...
              <h2 className="font-heading text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight">{featured.title}</h2>
              <div className="w-16 h-1 bg-gold mb-6" />
              <p className="text-white/50 text-lg leading-relaxed mb-8 max-w-xl">{featured.description}</p>
              <Link to={`/read/${featured.id}`} className="btn-shine-wrap inline-flex items-center gap-3 bg-gold text-black px-10 py-4 text-sm tracking-[0.15em] uppercase font-semibold hover:bg-gold-light transition-all duration-300 hover:scale-105">
                <BookOpen size={18} /> Read the Full Edition <ArrowRight size={16} />
              </Link>
            </ScrollReveal>
          </div>
      ...
      <section className="py-24 px-6">
        <ScrollReveal className="max-w-6xl mx-auto">
          <SectionLabel>Wisdom</SectionLabel>
          <h2 className="font-heading text-4xl md:text-5xl font-bold text-white text-center mb-14">Voices of Commerce</h2>
          <div className="grid md:grid-cols-3 gap-6">
            {voices.map((v, i) => (
              <ScrollReveal key={i} delay={i * 120}>
                <div className="bg-black/40 border border-gold/8 p-8 relative hover:border-gold/25 transition-all duration-500 h-full">
                  <Quote className="text-gold/30 mb-4" size={28} />
                  <p className="font-heading text-white/80 text-lg leading-relaxed italic mb-6">{v.quote}</p>
                  <div className="pt-4 border-t border-gold/10">
                    <span className="text-gold font-heading font-semibold block">{v.author}</span>
                    <span className="text-white/30 text-xs">{v.role}</span>
                  </div>
      ...
      <section className="py-20 px-6 bg-navy/20">
        <ScrollReveal className="max-w-5xl mx-auto grid grid-cols-3 gap-8 text-center">
          <div>
            <div className="font-heading text-4xl md:text-5xl font-bold text-gold"><CountUp end={4} /></div>
            <span className="text-white/40 text-xs tracking-wider uppercase mt-2 block">Editions Published</span>
          </div>
          <div>
            <div className="font-heading text-4xl md:text-5xl font-bold text-gold"><CountUp end={50} suffix="+" /></div>
            <span className="text-white/40 text-xs tracking-wider uppercase mt-2 block">Student Contributors</span>
          </div>
          <div>
            <div className="font-heading text-4xl md:text-5xl font-bold text-gold"><CountUp end={8} /></div>
            <span className="text-white/40 text-xs tracking-wider uppercase mt-2 block">Core Topics</span>
          </div>
        </ScrollReveal>
      </section>
      ...
      <section className="py-20 px-6">
        <ScrollReveal className="max-w-6xl mx-auto">
          <SectionLabel>Editions</SectionLabel>
          <h2 className="font-heading text-4xl md:text-5xl font-bold text-white text-center mb-12">Recent Editions</h2>
          <div className="grid md:grid-cols-3 gap-8">
            {published.slice(0, 3).map((ed, i) => (
              <ScrollReveal key={ed.id} delay={i * 120}>
                <EditionCard edition={ed} />
              </ScrollReveal>
            ))}
          </div>
